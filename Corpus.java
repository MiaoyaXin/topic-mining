...
public class Corpus
{
     ...
     * @throws IOException
     */
    public static int[] loadText(List<String> text, Vocabulary vocabulary) throws IOException
    {
        List<Integer> wordList = new LinkedList<Integer>();
        for (String wd: text)
        {
            if (wd.trim().length() < 2) continue;
            Integer id = vocabulary.getId(wd);
            if (id != null) wordList.add(id);
        }
        int[] result = new int[wordList.size()];
        int i = 0;
        for (Integer integer : wordList)
